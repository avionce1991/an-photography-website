...
const Header = () => {
  ...
  return (
    <>
        <div dangerouslySetInnerHTML={{ __html: '<!-- COMPONENT: Header -->' }} />
        ...
        <Link
          to={localePath("/")}
          className="flex items-center gap-2 md:gap-3 justify-start md:justify-center flex-1 pl-8 md:pl-0"
        >
      ...
      <div
        className={`fixed inset-0 z-[100] bg-background transition-all duration-500 flex flex-col items-center justify-center ${menuOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
          }`}
      >
        <button
        ...
        <nav className="flex flex-col items-center gap-8">
          {navItems.map((item) => (
            <Link
              ...
              to={item.path}
              onClick={() => setMenuOpen(false)}
              className={`font-serif text-4xl md:text-5xl font-light italic tracking-wide transition-opacity hover:opacity-60 ${location.pathname === item.path ? "opacity-100" : "opacity-70"
                }`}
            >
              {item.label}
            </Link>
